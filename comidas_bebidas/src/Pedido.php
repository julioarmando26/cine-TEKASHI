...
class Pedido{
    ...
    public function registrarDetalle($_params){
        $sql = "INSERT INTO `detalles_pedidos`(`pedido_id`, `alimentos_id`, `precio`, `cantidad`) 
        VALUES (:pedido_id,:alimentos_id,:precio,:cantidad)";

        $resultado = $this->cn->prepare($sql);

        $_array = array(
            ":pedido_id" => $_params['pedido_id'],
            ":alimentos_id" => $_params['alimentos_id'],
    ...
    public function mostrarDetallePorIdPedido($id)
    {
        $sql = "SELECT 
                ...
                INNER JOIN alimentos pe ON pe.id= dp.alimentos_id
